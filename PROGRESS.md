...
- [x] **Quote form wired to Zapier (2026-09-04, SMS confirmed live 2026-09-06)** — `/api/quote` no longer just holds a placeholder; `ZAPIER_QUOTE_WEBHOOK_URL` is set in Vercel production and points at Dylan's real Zap (Catch Hook → SMS + backup sheet). Sends the exact flat `{name, phone, details}` shape the Zap expects (`details` is service type + city + email + free-text notes concatenated). Call is fire-and-forget via `after()` from `next/server` — never blocks or fails the visitor's success confirmation, errors are logged server-side only. Form itself no longer opens `mailto:` at all; shows an in-page "Request received!" confirmation or, on failure, a call/text fallback. **Gotcha found 2026-09-06:** the Catch Hook trigger returns `{"status":"success"}` the instant it receives a payload regardless of whether the Zap is published — several test submissions looked "successful" from the webhook response but produced no SMS because the Zap itself was still off/draft, so Zap History showed zero runs. Fix was simply turning the Zap on. Confirmed end-to-end after: real POST to the production `/api/quote` route → Zap History shows an all-green run → SMS received on phone.
